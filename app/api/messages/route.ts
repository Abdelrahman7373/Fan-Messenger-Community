...
export async function POST(request: Request) {
    try {
        ...
        if(!currentUser?.id || !currentUser?.email){return new NextResponse('Unatuhorized', {status:401})}

        const newMessage = await prisma.message.create({
            data:{body:message, image: image, conversation:{connect:{id:conversationId}},sender:{connect:{id:currentUser.id}},seen:{connect:{id:currentUser.id}}},
