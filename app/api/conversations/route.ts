...
export async function POST(
    ...
    try {
        ...
        if(isGroup && (!members || members.length <2 || !name)){return new NextResponse('Not enough members!!', {status:400})};
        ...
        if(isGroup) {
            const newConversation = await prisma.conversation.create({
                data: {
                    name,isGroup,users:{
                        connect:[
                            ...members.map((member: {value: string}) => ({id: member.value,})),{id: currentUser.id}
                        ]
                    }
                }, include: {users:true},
            });
